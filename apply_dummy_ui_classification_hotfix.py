...
BACKUP_ROOT = ROOT / ".dummy_ui_classification_backups"
MANIFEST_NAME = "manifest.json"
MARKER = "NETZENTGELT_DUMMY_UI_CLASSIFICATION_V1_20260609"
...
EXPECTED_BLOB_SHA = {
    ...
    "scripts/verify_dummy_locomotive_hardening.py": "23786c2ec6d2bb1d857c3a77e2e7f9ef672a0e0d",
...
def upsert_dummy_locomotive_mapping(*, loco_no: str, reason: str, changed_by: str) -> str:
    # Controller-Klassifikation auditierbar in den zentralen Dummy-Katalog übernehmen.
    ...
    if not cleaned_reason:
        raise ValueError("Bitte eine nachvollziehbare Begründung angeben.")
...
def patch_manual_ui(text: str) -> str:
    ...
    old_block = """        override_value = st.text_input(
            "Neuer Wert",
...
"""
    new_block = """        if is_dummy_action:
            st.info(
                "Die Lok wird als Dummy-/Planungslok in den zentralen Katalog aufgenommen. "
                "Nach der Neuberechnung wird sie aus Timeline, GAPs, Quality Gate und Exporten ausgeschlossen."
            )
            override_value = ""
            classification_code = ""
        else:
...
"""
    text = replace_once(text, old_block, new_block, "manual ui conditional dummy form")
    text = replace_once(
        text,
        ...
        "manual ui dummy submit labels",
    )
    dummy_submit = """    if override_type == "MARK_DUMMY_LOCOMOTIVE":
        if not target_loco_no.strip():
            st.error("Bitte eine Loknummer angeben.")
            return
        actor = created_by.strip() or getpass.getuser()
        try:
            catalog_action = upsert_dummy_locomotive_mapping(
        ...
        st.success(
            f"Lok {target_loco_no.strip()} wurde als Dummy-/Planungslok gespeichert. "
            f"Katalogaktion: {catalog_action}."
        )
        if save_and_rebuild:
            with st.status("Dummy-Katalog wird gespeichert und die Daten werden sicher neu berechnet ...", expanded=True) as status:
...
def patch_verifier(text: str) -> str:
    ...
    return replace_once(
        ...
        '    "00000000001-8",\n}',
        '    "00000000001-8",\n    "91806189000-3",\n}',
        "verifier known dummy locomotive",
    )
...
PATCHERS = {
    ...
    "scripts/verify_dummy_locomotive_hardening.py": patch_verifier,
...
def build_patched(root: Path, test_mode: bool) -> dict[str, bytes]:
    ...
    for relative, patcher in PATCHERS.items():
        ...
        patched[relative] = encode_text(updated, has_bom, newline)
...
def verify_expected_state(root: Path) -> None:
    checks = {
        ...
        "scripts/verify_dummy_locomotive_hardening.py": ['"91806189000-3",'],
        "data/01_mapping/dummy_locomotives.csv": ["91806189000-3;"],
        "scripts/test_dummy_locomotive_ui_classification.py": ["NETZENTGELT_DUMMY_UI_CLASSIFICATION_TEST_V1_20260609"],
        "scripts/verify_dummy_locomotive_ui_classification.py": ["NETZENTGELT_DUMMY_UI_CLASSIFICATION_VERIFY_V1_20260609"],
    }
    ...
    for relative in [
        ...
        "scripts/verify_dummy_locomotive_hardening.py",
...
def verify(root: Path) -> int:
    verify_expected_state(root)
    print("OK: Marker, Katalogeintrag und Python-Syntax verifiziert.")
    return 0
