...
BACKUP_ROOT = ROOT / ".dummy_ui_classification_v3_backups"
MANIFEST_NAME = "manifest.json"
MARKER = "NETZENTGELT_DUMMY_UI_CLASSIFICATION_V2_20260609"
MUTABLE_FILES = {"data/01_mapping/dummy_locomotives.csv"}
...
def validate_mutable_catalog(relative: str, raw: bytes) -> None:
    """Pflegbare Konfigurationsdatei strukturell prüfen, ohne lokale Einträge zu überschreiben."""
    text, _, _ = decode_text(raw)
    lines = [line.strip() for line in text.split("\n") if line.strip()]
    if not lines:
        raise RuntimeError(f"Pflegbarer Dummy-Katalog ist leer: {relative}")
    if lines[0].lower() != "loco_no;reason;active_flag":
        raise RuntimeError(
            f"Unerwarteter CSV-Header in '{relative}'. Erwartet: loco_no;reason;active_flag. "
            f"Vorhanden: {lines[0]}"
        )
...
def validate_baseline(relative: str, raw: bytes, test_mode: bool) -> None:
    if relative in MUTABLE_FILES:
        validate_mutable_catalog(relative, raw)
        return
    if test_mode:
        return
