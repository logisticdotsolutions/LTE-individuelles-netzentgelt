...
3. `Z01` is a holder-side export and must not be generated per PerformingRU. LTE Holding is the only holder-side sender for the locomotive assignments. PerformingRU remains relevant only as row-level assignment information for the target `Nutzer-vEns*`.

   RailCube company label:
   - `LTE Logistik- und Transport-GmbH (Holding)`

   LTE Holding market partner IDs:
   - `1900100300393`
   - `1900100400391`

   Confirmed export rule:
   - Generate two separate Z01 files.
   - Both files contain the identical set of exportable DE-relevant locomotive segments in the selected period.
   - The files differ only by the header field `Marktpartner ID` and by the file name.
...
1. Verify the two fixed LTE-Holding Z01 downloads locally.
