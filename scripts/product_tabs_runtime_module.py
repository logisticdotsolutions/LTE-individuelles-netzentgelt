...
EXPORT_TAB_RENUMBERED_LABEL = EXPORT_TAB_LABEL
...
def _visible_tab_labels(labels: Sequence[object]) -> tuple[list[object], int | None, int | None]:
    ...
    visible_labels.insert(loco_index + 1, EXPORT_TAB_LABEL)
    visible_labels.insert(loco_index + 2, TECH_TAB_LABEL)

    return visible_labels, None, None
...
def install_product_tabs_runtime():
    ...
    def patched_tabs(labels: Sequence[object], *args, **kwargs):
        visible_labels, _, _ = _visible_tab_labels(labels)
