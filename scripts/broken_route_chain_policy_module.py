...
BROKEN_ROUTE_CHAIN_POLICY_MARKER = "NETZENTGELT_BROKEN_ROUTE_CHAIN_NO_LTE_POLICY_V1_20260630"
DISABLED_BROKEN_CHAIN_RULE_IDS = ("R010", "R010.5", "R016")
...
_RULE_CATALOG_ROWS = {
    "R010": (
        "TIMELINE",
        "DISABLED",
        "Deaktiviert: Gebrochene Ortskette wird fachlich als Keine LTE Zuordnung behandelt und ist kein Fehler.",
    ),
    "R010.5": (
        "TIMELINE",
        "DISABLED",
        "Deaktiviert: Kurze gebrochene Ortskette wird fachlich als Keine LTE Zuordnung behandelt und ist kein Hinweis.",
    ),
    "R016": (
        "TIMELINE",
        "DISABLED",
        "Deaktiviert: GAP-only-/Keine-LTE-Zuordnungstage erzeugen keinen manuellen Prueffall.",
    ),
}
...
def is_no_lte_assignment_marker(*values: object) -> bool:
    """Return True for explicit no-LTE/broken-route-chain markers in UI text."""
...
def _ensure_disabled_catalog_row(con, rule_id: str) -> None:
    if not _table_exists(con, "cfg_dq_rule_catalog"):
        return
    if not _has_columns(
        con,
        "cfg_dq_rule_catalog",
        ["rule_id", "rule_group", "severity_policy", "description", "active_flag"],
    ):
        return

    rule_group, severity_policy, description = _RULE_CATALOG_ROWS[rule_id]
    con.execute(
        """
        delete from cfg_dq_rule_catalog
        where rule_id = ?
        """,
        [rule_id],
    )
    con.execute(
        """
        insert into cfg_dq_rule_catalog (
            rule_id,
            rule_group,
            severity_policy,
            description,
        ...
        """,
        [rule_id, rule_group, severity_policy, description],
    )


def disable_broken_route_chain_rules(con) -> None:
    """Deactivate broken-route-chain findings without removing legacy rule code."""
    ...
    for rule_id in DISABLED_BROKEN_CHAIN_RULE_IDS:
        _ensure_disabled_catalog_row(con, rule_id)
...
def neutralize_broken_route_chain_quality_gate(con) -> None:
    """Remove GAP/broken-chain impact from export gate status and reasons."""
    gap_metric_defaults = {
    ...
    for table_name in ["core_loco_day_coverage", "dq_export_gate", "dq_export_gate_ru"]:
        ...
        if assignments:
            con.execute(
                f"update {qident(table_name)} set " + ", ".join(assignments)
            )
...
def patch_phase6d_gap_only_rule(module) -> None:
    if getattr(module, "_BROKEN_ROUTE_CHAIN_R016_POLICY_PATCHED", False):
        return
    if not hasattr(module, "insert_gap_only_day_findings_phase6d"):
        return

    def disabled_insert_gap_only_day_findings_phase6d(
        con,
        run_id: str,
        loco_filter: "frozenset[str] | None" = None,
    ) -> None:
        _ = run_id, loco_filter
        disable_broken_route_chain_rules(con)
        neutralize_broken_route_chain_quality_gate(con)
        print("Phase 6D: R016 deaktiviert - GAP-only gilt als Keine LTE Zuordnung.")

    module.insert_gap_only_day_findings_phase6d = disabled_insert_gap_only_day_findings_phase6d
