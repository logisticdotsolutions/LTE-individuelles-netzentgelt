...
def _build_matches(con) -> None:
    con.execute(
        ...
        join core_loco_timeline t
         ...
         and (
            (
                o.source_table is not null and o.source_row_id is not null
            ...
            or
            (
                o.loco_no is not null and o.period_start_utc is not null and o.period_end_utc is not null
...
def _write_audit(con, run_id: str) -> None:
    ...
    con.execute(
        ...
        select
            ? as phase_id,
            ? as run_id,
            trim(o.override_id) as override_id,
            nullif(trim(o.target_loco_no), '') as loco_no,
            try_cast(nullif(trim(o.target_actual_departure_utc), '') as timestamp) as period_start_utc,
            try_cast(nullif(trim(o.target_actual_arrival_utc), '') as timestamp) as period_end_utc,
            nullif(trim(o.target_source_table), '') as source_table,
            nullif(trim(o.target_source_row_id), '') as source_row_id,
            count(m.override_id) as matched_timeline_rows,
            case when count(m.override_id) > 0 then 'APPLIED' else 'NO_MATCH' end,
            case when count(m.override_id) > 0 then ? else 'Keine passende GAP-Zeile gefunden. Override bleibt auditierbar sichtbar.' end,
            current_timestamp
        from cfg_manual_overrides_effective o
        left join tmp_no_lte_gap_matches m on m.override_id = trim(o.override_id)
        where upper(trim(coalesce(o.override_type, ''))) = 'CLASSIFY_GAP'
          and upper(trim(coalesce(o.classification_code, ''))) = ?
        group by o.override_id, o.target_loco_no, o.target_actual_departure_utc,
                 o.target_actual_arrival_utc, o.target_source_table, o.target_source_row_id
        """,
...
def apply_no_lte_gap_release(con, run_id: str) -> int:
    """Mark explicitly confirmed no-LTE GAPs as non-blocking while keeping them audit-visible."""
    _ensure_audit_table(con)
    ...
    if matched:
        con.execute(
            ...
            where upper(trim(coalesce(target.row_type, ''))) = 'GAP'
              and exists (
                    select 1 from tmp_no_lte_gap_matches m
                    where m.loco_no is not distinct from target.loco_no
                      and m.period_start_utc is not distinct from target.period_start_utc
                      and m.period_end_utc is not distinct from target.period_end_utc
                      and m.source_table is not distinct from target.source_table
                      and m.source_row_id is not distinct from cast(target.source_row_id as varchar)
              )
            """,
        ...
        if table_exists(con, "dq_findings"):
            con.execute(
                ...
                where exists (
                    select 1 from tmp_no_lte_gap_matches m
                    where m.loco_no is not distinct from target.loco_no
                      and target.period_start_utc < m.period_end_utc
                      and target.period_end_utc > m.period_start_utc
                )
                and (upper(trim(coalesce(target.row_type, ''))) = 'GAP' or upper(trim(coalesce(target.rule_id, ''))) = 'R016')
                """,
                [RELEASE_MESSAGE],
            )
